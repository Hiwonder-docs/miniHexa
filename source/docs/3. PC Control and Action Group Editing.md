...
2) After the hotspot connection succeeds, open the PC software and click **Scan Device**. The software automatically matches miniHexa.
...
2) Posture Control: controls the posture and center of gravity of miniHexa.
...
6. Click **Download** to download the action group to the `miniHexa` robot.
...
6) Click the "<img src="../_static/media/chapter_3/section_6/media/image7.png" class="inline-icon"  />" button to run the newly integrated action group online once.
...
3. Tap **Action Group** on the main interface.
